...
Note.prototype = {
	...
	getEnharmonicFlat: function() {
		if (this.accidental === sharp || this.letter === 'B' || this.letter === 'E') {
			var newNote = new Note(this.getNextLetterNote(this.letter), flat);
			newNote.up = this.up;
	...
	getEnharmonicSharp: function() {
		if (this.accidental === flat || this.letter === 'C' || this.letter === 'F') {
			var newNote = new Note(this.getPreviousLetterNote(this.letter), sharp);
			newNote.up = this.up;
			newNote.down = this.down;
			return newNote;
		} else {
