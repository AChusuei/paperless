Paperless = {
	printTemplate: function(template) {
		...
		template.order.forEach(function (sectionNameRaw) {
			...
			if (sectionNamesPrinted[sectionName]) {

			} else {
				if (!section.lyrics) {
					section.lyrics = [section.progression[0].map(function(c) { return '^  ' }).join()];
				}
				section.lyrics.forEach(function(line, index) {
					var chordLine = line.replace(/\^./g, '\^') + '    '; // extra spaces for over extended regex matches 
					chordLine = chordLine.replace(/[^\^]/g, ' ');
					var chords = section.progression[index];
					chords.forEach(function(position) {
						var chordString = scale.getChordAtPosition(position).toString();
						var length = chordString.length - 1;
						var chordRegex = new RegExp('\\^.{' + length + '}');
						chordLine = chordLine.replace(chordRegex, chordString);
					})
